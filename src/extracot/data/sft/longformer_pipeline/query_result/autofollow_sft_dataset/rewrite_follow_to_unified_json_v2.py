...
If --tokenize_100 is provided, ratio 100 will follow the same format as 020–080
(i.e., input: question/query + " " + <COMP_100>, output: <COMP_100> + "\\n" + output).
...
def find_ratio_file(dir_path: Path, ratio: str) -> Optional[Path]:
    """
...
def first_nonempty(obj: dict, keys: list[str]) -> Optional[str]:
    for k in keys:
        v = obj.get(k)
...
def rewrite_sample_ratio_20_80(obj: dict, token: str) -> Optional[dict]:
...
def rewrite_sample_ratio_100(obj: dict) -> Optional[dict]:
...
def main():
    ap = argparse.ArgumentParser(
        description="Rewrite rxxx.jsonl files and merge into a single shuffled JSON dataset."
    )
    ...
    ap.add_argument("--keep_zero_pad", default=False, action="store_true",
                    help="Use <COMP_020> instead of <COMP_20> style tokens")
    ap.add_argument("--tokenize_100", default=True, action="store_true",
                    help="Also add <COMP_100> to ratio=100 items (input ends with token; output prepends token + newline)")
    args = ap.parse_args()
    ...
    for ratio in RATIOS:
        ...
        for obj in load_jsonl(path):
            ...
            # Decide which rewrite path to use
            if is_hundred and not args.tokenize_100:
                new_obj = rewrite_sample_ratio_100(obj)
            else:
                # For 020–080 and (optionally) 100 when --tokenize_100 is set
        ...
        stats["per_ratio"][ratio] = {
            ...
            "skipped": skipped,
            "token_used": tok if (not is_hundred or args.tokenize_100) else None,
        }
        sys.stderr.write(
            f"[INFO] {ratio}: read={read_count} rewritten={rewritten_count} skipped={skipped} "
            f"token={'on' if ((not is_hundred) or args.tokenize_100) else 'off'}\n"
        )
