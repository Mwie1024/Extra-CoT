...
- γ 注入方式：
  ...
  * special：单token（如 <COMP_70>，或 r==2.0 → <COMP_AUTO>）
- 可选：按 γ 等比缩放 max_new_tokens（--len_ctrl proportional）以对齐 TokenSkip “一比率一上限”的评测套路
- COT 长度统计：优先 <think>…</think>，其次 \boxed{ 之前，再退化为整段
- 模型加载（本版增强）：
  * 先加载“基模”
  * 再按 --lora_mode 选择是否/如何加载 LoRA：off / attach / merge
  * 当以 special 注入新增特殊 token 时，自动对模型 resize_token_embeddings 以避免 OOV/越界
"""
...
def build_user_text(query: str,
    ...
    ratio_injection:
      ...
      - 'special'  ：单token，如 <COMP_70>；当 ratio==2.0 时用 <COMP_AUTO>
    """
...
def maybe_add_specials(tokenizer, tokens: List[str]) -> int:
...
def load_model_tokenizer(model_path: str,
                         ...
                         dtype: str = "fp16",
                         device_map: str = "auto",
                         lora_mode: str = "attach"):
    """
    lora_mode:
      - 'off'    : 不加载 LoRA（只用基模）
      - 'attach' : 基模上挂载 LoRA 分支（不合并），直接推理
      - 'merge'  : 将 LoRA 权重合并进基模并卸载 LoRA 分支，之后以纯模型推理
    """
    # 1) tokenizer
    ...
    if ratio_injection == "special" and ratio <= 1.0:
        added += maybe_add_specials(tok, [f"<{special_prefix}{int(round(ratio*100))}>"])
    if ratio_injection == "special" and ratio == 2.0:
        added += maybe_add_specials(tok, ["<COMP_AUTO>"])
    ...
    # 若新增了 special tokens，需同步扩展 embedding
    if added > 0:
        base.resize_token_embeddings(len(tok))
        print(f"[model] resized token embeddings to {len(tok)} after adding {added} special tokens")

    # 3) LoRA（可选）
    lora_path = adapter_path
    if lora_path and str(lora_path).lower() not in ["", "none"] and lora_mode != "off":
        print(f"[lora] loading LoRA from: {lora_path}  (mode={lora_mode})")
        ...
        if lora_mode == "merge":
            # 合并 LoRA 权重进基模，并卸载 LoRA 分支（推理时就是一个纯模型）
            base = peft.merge_and_unload()
            print("[lora] merged and unloaded LoRA")
        else:
            # 直接挂载 LoRA：保持为 PeftModel，用 LoRA 分支参与推理
            base = peft
...
def main():
    ...
    # 兼容旧参：adapter_path；并新增 lora_path（若两者同时提供，优先 lora_path）
    ap.add_argument("--adapter_path", default=None)
    ap.add_argument("--lora_path", default=None,
                    help="LoRA 适配器路径；若与 --adapter_path 同时提供，优先使用本参数")

    ap.add_argument("--model_type", choices=["qwen","llama3"], default="qwen")

    # LoRA 载入模式
    ap.add_argument("--lora_mode", choices=["off","attach","merge"], default="attach",
                    help="off: 不加载 LoRA；attach: 基模上挂载 LoRA（不合并）直接推理；merge: 合并 LoRA 后推理")
    ...
    # 加载模型与 tokenizer
    # 若同时提供 --lora_path 与 --adapter_path，优先 lora_path
    effective_adapter = None
    if args.lora_path not in (None, "", "none"):
        effective_adapter = args.lora_path
    elif args.adapter_path not in (None, "", "none"):
        effective_adapter = args.adapter_path
