...
# 宽松的 “The answer is: …” 获取 gold
GT_LAST    = re.compile(r"The\s+answer\s+is\s*:\s*(.+?)(?=$|\n)", flags=re.I)
# 配对提取最后一个 \boxed{...}
def _find_all_boxed_balanced(text: str) -> List[Tuple[int,int,str]]:
    if not text: return []
    s = text; out = []; i = 0; needle = r"\boxed{"
    while True:
        j = s.find(needle, i)
        if j == -1: break
        k = j + len(needle)
...
def extract_last_boxed(s: str) -> Optional[str]:
    spans = _find_all_boxed_balanced(s or "")
    return spans[-1][2].strip() if spans else None
...
def answers_equal(pred: Optional[str], gold: Optional[str]) -> bool:
    ...
    p, g = _strip_latex(pred), _strip_latex(gold)
...
def choose_target_first_monotonic(pass_map: Dict[float,bool]) -> Tuple[Optional[float], bool]:
    """
    返回 (chosen_r, non_mono_flag)。严格单调：选通过列表中最小 r，
    且对所有 r' >= r 都必须 True；否则返回 (None, True) 记为 non-mono。
    """
...
def main():
    ...
    ap.add_argument("--bins", default="20:0.10-0.30,40:0.30-0.50,60:0.50-0.70,80:0.70-0.90,100:0.90-1.10")
    ap.add_argument("--seed", type=int, default=42)

    # 25k 配额（可改），或用你给的 31.7k 比例做“参考分布”另行统计
    ap.add_argument("--quotas", default="20:1000,40:1000,60:1000,80:1000,100:1000",
                    help="per-bin target counts summing to total (e.g., 25k)")
    ap.add_argument("--allow_non_mono_bins", default="80,100",
                    help="comma-list of bin labels that may use non-monotonic fillers if shortage")
    ap.add_argument("--shuffle_within_bin", action="store_true",
                    help="randomize candidates in a bin before closeness sort")
    ap.add_argument("--min_ref_think", type=int, default=80,
                    help="drop items whose r100 <think> tokens < this (too brittle).")
    args = ap.parse_args()
    ...
    # ---- 读入与并集索引（对每 ratio 以 id 去重；默认保留 <think> 更长的那条）----
    tag2r = {"020":0.2,"040":0.4,"060":0.6,"080":0.8,"100":1.0}
    ...
    # ---- 遍历 id：构建 pass_map / actual_map / 选择难度 / 按 actual 分桶 ----
    kept = []
    ...
    for sid, bobj in base.items():
        ...
        for r in TARGETS:
            ...
            pass_map[r] = ok
            if ok:
                cur_think = extract_think_inner(e.get("model_output",""))
                cur_len = max(1, count_tokens(cur_think))
                cur_len_map[r] = cur_len
        ...
        # 计算本样本的“入桶 actual 与 current_length”
        if chosen_r is None:
            # non-mono：用产生“最小 actual”的那个目标档
            if not actual_map:
                continue
            rr_min, act_min = min(actual_map.items(), key=lambda kv: kv[1])
            cur_len = int(cur_len_map.get(rr_min, max(1, int(act_min*base_len))))
            ...
            kept.append({
                ...
                "current_length": cur_len,                # 选定档位对应的 <think> 长度
                "monotonic": False,
                "actual_ratio": float(act_min),
                "bin_label": lbl, "bin_range": [L,R],
                "source": "non_mono"
            })
            continue

        # 有严格单调解：用 chosen_r 的 actual 入桶
        act = actual_map.get(chosen_r, None)
        if act is None:
            # 极端兜底（理论上少见）
            if not actual_map: continue
            chosen_r, act = min(actual_map.items(), key=lambda kv: kv[1])
        cur_len = int(cur_len_map.get(chosen_r, max(1, int(act*base_len))))
        ...
        kept.append({
            ...
            "monotonic": True,
            "actual_ratio": float(act),
            "bin_label": lbl, "bin_range": [L,R],
            "source": "mono"
        })

    # ---- 按 bin 聚合，区内排序：单调优先 + 距中点更近优先 ----
    ...
    # 第一轮：mono 填到 min(target, mono_supply)
    ...
    # 仍有缺口：第三轮全局按“其他桶的可用剩余”重分目标（不跨桶抽，只调整目标）
    ...
    meta = {
        ...
        "supply": {"mono": supply_mono, "non_mono": supply_non, "all": supply_all},
        "quotas_request": quotas,
        "allow_non_mono_bins": sorted(list(allow_non_mono)),
        "final_take": final_take,
        "per_bin_used": per_bin_used,
        "sizes": {"target_total": total_target, "sampled": len(sampled), "unique_ids": len(used)},
        "policy": "bucket by ACTUAL ratio of chosen r; difficulty = strict target-first; prefer mono; per-bin no-dup by id",
    }
